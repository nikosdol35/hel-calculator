...
EXPLANATIONS: dict[str, str] = {
    ...
    "plot_p_intro_pre": (
        "How effective is the laser against targets approaching "
        "from different angles? A head-on attacker is the easiest "
        "engagement; a target flying past sideways is the hardest. "
    ...
    "plot_p_intro": (
        ...
        "over time during one engagement. A rising curve means "
        "the target is closing in — the laser spot tightens, "
        "intensity grows. A falling curve means the target is "
        "moving away — the spot widens, intensity drops. Curves "
        "end either when the target reaches your minimum-engagement "
        "range (the head-on case), turns away at closest approach "
        "(the angled cases), or runs out of the time window (the "
        "perpendicular case). Your current scenario is the bold "
        "white line with a star at engagement-end. The reference "
        "curves use a simplified physics path for fast comparison; "
        "your white line uses the full physics. Compare the "
        "shapes of the curves, not the exact numbers."
    ),
    "plot_o_intro_pre": (
    ...
    "plot_n_intro_pre": (
        ...
        "(τ_BT below ½ dwell, comfortable margin), amber = risky "
        "(τ_BT between ½ dwell and dwell, getting tight), red = "
        "infeasible (τ_BT exceeds dwell or laser too weak to reach "
        "burn-through). The white star is your scenario. Drag the "
        "chart to zoom in; double-click to reset."
    ),
    "plot_n_intro": (
        "Approximate scaling — the curve uses a fast lumped-mass "
        "τ_BT for the sweep, while the white star is your scenario's "
        "PDE-accurate value (matches the headline τ_BT). The curve "
        "extends past the dwell line into the infeasible region so "
        "you can see the trajectory; in the red zone the laser "
        "can't actually reach burn-through (radiation losses outpace "
        "absorption), but the climbing curve shows what time would "
        "be needed if those losses were ignored."
    ),
    "plot_k_intro_pre": (
